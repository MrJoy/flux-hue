def chunk(items, step_size = 0.1, digits = 1)
    ...
    items.each do |item|
        ...
        (start_bucket..start_bucket+duration).step(step_size) { |x|
            x = x.round(digits)
            chunks_out[x] ||= []
...
bucketed.each do |url,events|
    events.each_with_index do |event, index|
        if event[:action] == "END" and index > 0 and events[index-1][:action] == "BEGIN"
            good_events[url] ||= []
            good_events[url].push({
                start: events[index-1][:time],
